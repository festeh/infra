...
`dimalip.in` site are active; every other retained service is absent. Caddy has
deny-by-default catch-alls plus the reviewed `ai.dimalip.in`, `rss.dimalip.in`,
`dimalip.in`, and `www.dimalip.in` routes. Listener and route values record the
reviewed production shape; every role must revalidate them before enabling the
service.
...
| Papujki | `festeh/papujki` artifact | `127.0.0.1:3333` | `papujki.space` | Caddy |
